...
The state space has 37 dimensions and contains the agent's velocity, along with ray-based perception of objects around agent's forward direction. More detailed state description below:
...
* Follow the instructions in `Navigation.ipynb` to better understand the environment api!  
* `model.py` includes the QNetwork object definition
* `dqn_agent.py` includes an agent which uses the QNetwork for value function estimation
* `main.py` runs the training and saves the model parameters
* `run_simulation.py` loads the trained model `results/checkpoint.pth` and runs a simulation
